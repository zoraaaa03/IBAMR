...
main(int argc, char** argv)
{
    ...
    // Test 4: Large iteration numbers and edge cases
    pout << "\n=== Test 4: Large Iteration Numbers and Edge Cases ===" << std::endl;
    {
        const std::string test_dir = "test_restart_large";
        std::vector<int> large_iterations = {999995, 999996, 999997, 999998, 999999};
        
        create_test_restart_dirs(test_dir, large_iterations);
        
        try
        {
            RestartCleaner cleaner(test_dir, 3, "KEEP_RECENT_N", false);
            
            auto iterations_before = cleaner.getAvailableIterations();
            pout << "Large iterations before cleanup: ";
            for (int iter : iterations_before) pout << iter << " ";
            pout << std::endl;
            
            cleaner.cleanup();
            
            auto iterations_after = cleaner.getAvailableIterations();
            pout << "Large iterations after cleanup: ";
            for (int iter : iterations_after) pout << iter << " ";
            pout << std::endl;
            
            // Should keep the 3 highest
            std::vector<int> expected = {999997, 999998, 999999};
            if (iterations_after == expected)
            {
                pout << "Test 4 PASSED: Correctly handled large iteration numbers" << std::endl;
            }
            else
            {
                pout << "FAILED: Large iteration number handling error" << std::endl;
                test_failures++;
            }
        }
        catch (const std::exception& e)
        {
            pout << "FAILED: Exception in large numbers test: " << e.what() << std::endl;
            test_failures++;
        }
    ...
    // Test 5: Database constructor functionality
    pout << "\n=== Test 5: Database Constructor Functionality ===" << std::endl;
    {
        ...
        try
        {
            ...
            else
            {
                ...
                if (iterations_after == expected)
                {
                    pout << "Test 5 PASSED: Database constructor worked correctly" << std::endl;
                }
                else
                {
    ...
    pout << "\n=== Test 6: Error Handling and Edge Cases ===" << std::endl;
    {
        try
        {
            ...
            if (test_failures == 0)
            {
                pout << "Test 6 PASSED: Error handling works correctly" << std::endl;
            }
        }
        catch (const std::exception& e)
        {
    ...
    // Test 7: MPI environment compatibility
    pout << "\n=== Test 7: MPI Environment Compatibility ===" << std::endl;
    {
        try
        {
            ...
            pout << "Test 7 PASSED: MPI environment compatibility verified" << std::endl;
        }
        catch (const std::exception& e)
        {
    ...
    if (test_failures == 0)
    {
        ...
        pout << "  - Large iteration number handling" << std::endl;
